...
NS_LOG_COMPONENT_DEFINE ("Controller2");

namespace ns3 {

NS_OBJECT_ENSURE_REGISTERED (Controller2);

// register NS-3 type
TypeId
Controller2::GetTypeId ()
{
  static TypeId tid = TypeId ("Controller2")
    .SetParent<ndn::App> ()
    .AddConstructor<Controller2> ()

    .AddAttribute ("Prefix", "Requested name",
                   StringValue ("/prefix/sub2"),
                   ndn::MakeNameAccessor (&Controller2::m_name),
                   ndn::MakeNameChecker ())
    ;
  return tid;
}
...
ndn::App::StartApplication ();
 ...
 // Simulator::Schedule (Seconds (1.0), &CustomApp::OnInterest, this);
  m_isRunning = true;
  Simulator::ScheduleNow (&Controller2::TimerSet,this);
  t=Simulator::Now ();
...
void
Controller2::OnInterest (Ptr<const ndn::Interest> interest)
{
long timeset;
...
Controller2::TimerSet();

}
// Processing when application is stopped
void
Controller2::StopApplication ()
{
  m_isRunning = false;
  // cleanup ndn::App
...
Ptr<ndn::Interest> interest4 = Create<ndn::Interest> ();
void Controller2::TimerSet(void)
{
if(m!=0.0)

{t=Seconds(3.5);
if(b)
{Controller2::SendInterest ();
//if(m==0.0)
//Simulator::Schedule (Seconds (1), &Controller::TimerSet, this);
if(m!=0.0)
Simulator::Schedule (Seconds (m), &Controller2::TimerSet, this); }
if(b==0)
{Controller2::SendInterest ();
b++;}
}
...
static int n=1;
void
Controller2::OnData (Ptr<const ndn::Data> contentObject)
{
