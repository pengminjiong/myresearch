...
NS_LOG_COMPONENT_DEFINE ("Controller1");

namespace ns3 {

NS_OBJECT_ENSURE_REGISTERED (Controller1);

// register NS-3 type
TypeId
Controller1::GetTypeId ()
{
  static TypeId tid = TypeId ("Controller1")
    .SetParent<ndn::App> ()
    .AddConstructor<Controller1> ()

    .AddAttribute ("Prefix", "Requested name",
                   StringValue ("/prefix/sub1"),
                   ndn::MakeNameAccessor (&Controller1::m_name),
                   ndn::MakeNameChecker ())
    ;
  return tid;
}
...
ndn::App::StartApplication ();
 ...
 // Simulator::Schedule (Seconds (1.0), &CustomApp::OnInterest, this);
  m_isRunning = true;
  Simulator::ScheduleNow (&Controller1::TimerSet,this);
  t=Simulator::Now ();
...
void
Controller1::OnInterest (Ptr<const ndn::Interest> interest)
{
long timeset;
...
Controller1::TimerSet();
}
// Processing when application is stopped
void
Controller1::StopApplication ()
{
  m_isRunning = false;
  // cleanup ndn::App
...
Ptr<ndn::Interest> interest3 = Create<ndn::Interest> ();
void Controller1::TimerSet(void)
{
if(m!=0.0)

{t=Seconds(3.5);
if(b)
{Controller1::SendInterest ();
//if(m==0.0)
//Simulator::Schedule (Seconds (1), &Controller::TimerSet, this);
if(m!=0.0)
Simulator::Schedule (Seconds (m), &Controller1::TimerSet, this); }
if(b==0)
{Controller1::SendInterest ();
b++;}
}
...
static int n=1;
void
Controller1::OnData (Ptr<const ndn::Data> contentObject)
{
